import turtle  # Импортируем модуль turtle для рисования и работы с графикой.
import time    # Импортируем модуль time для работы с задержками.
import random  # Импортируем модуль random для генерации случайных позиций еды.

# Настройки окна
window = turtle.Screen()          # Создаём игровое окно.
window.title("Змейка")            # Задаём заголовок окна.
window.bgcolor("black")           # Устанавливаем чёрный цвет фона.
window.setup(width=600, height=600)  # Устанавливаем размер окна 600x600 пикселей.
window.tracer(0)                  # Отключаем автоматическое обновление экрана, чтобы контролировать обновления вручную.

# Голова змейки
head = turtle.Turtle()            # Создаём объект головы змейки.
head.speed(0)                     # Устанавливаем скорость анимации головы (0 - максимальная).
head.shape("square")              # Устанавливаем форму головы змейки в виде квадрата.
head.color("white")               # Задаём цвет головы змейки - белый.
head.penup()                      # Отключаем рисование линий за змейкой.
head.goto(0, 0)                   # Устанавливаем начальную позицию головы в центре экрана (0, 0).
head.direction = "stop"           # Задаём начальное направление движения как "стоп" (змейка стоит на месте).

# Еда для змейки
food = turtle.Turtle()            # Создаём объект еды.
food.speed(0)                     # Устанавливаем скорость анимации для еды.
food.shape("circle")              # Устанавливаем форму еды в виде круга.
food.color("red")                 # Задаём красный цвет для еды.
food.penup()                      # Отключаем рисование линии за едой.
food.goto(0, 100)                 # Устанавливаем еду на начальную позицию (0, 100 пикселей по оси Y).
...
window.listen()                   # Сообщаем окну, что оно должно "слушать" нажатия клавиш.
window.onkey(go_up, "Up")          # Устанавливаем управление: нажатие клавиши "Вверх" вызывает функцию go_up().
window.onkey(go_down, "Down")      # Нажатие клавиши "Вниз" вызывает функцию go_down().
window.onkey(go_left, "Left")      # Нажатие клавиши "Влево" вызывает функцию go_left().
window.onkey(go_right, "Right")    # Нажатие клавиши "Вправо" вызывает функцию go_right().
